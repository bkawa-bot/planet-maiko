    ...
    try:
        ...
        for run in runs:
            ...
            for node in nodes:
                ...
                role = nr.agent_type
                # Compose this node's input from its upstream sources. A
                # "diff" producer hands off through the remote: push its
                # branch to origin and tell this step to fetch + review it
                # (the reviewer already knows how to fetch a ref and diff
                # against base, and leaves local comments). Every other
                # kind hands off its text artifact as a context section.
                blocks = []
                push_failed = False
                for src in inbound:
