...
def cartograph_repo():
    """Queue an AgentJob to cartograph the given repo. Atlas (the
    ...
    profile = (AgentProfile.query
               ...
               .first())
    if not profile:
        # Cartographer profile gets a fixed display name. The
        # default_display_name column on AgentType used to drive this
        # but only cartographer ever used it, so it's hardcoded here
        # in the cartographer-spawn path now.
    ...
    job = AgentJob(
        ...
        description="User-triggered cartograph via Playbook UI.",
        scope_repo=repo,
        priority="normal",
        created_by="user",
